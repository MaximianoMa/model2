...
def Year_simulation(D, T, T_week, t_peak, Load_max, load_pu, N_wt, WT_cap, wt_pu, T_wt, N_pv, PV_cap, pv_pu, T_pv,
    ...
    for m in range(D):
        ...
        Pabn_week = [cp.Variable() for _ in range(T_week)]  # 抽水蓄能的使用量
        Pgrid_week = cp.Variable(T_week)  # 电网的使用量
        ...
        for k in range(T_week):
            # 能量平衡约束
            Constraints.append(0 <= Pgrid_week[k])
            P_all_week[k] = Pgrid_week[k]
            ...
            for i in range(N_ESS_week):
                ...
                Pabn_week[k] = Pabn0_week[k] + (1 - Ita_week) * Pbc_week[i, k] + (1 - Ita_week) * Pbd_week[
                        i, k] / Ita_week
        ...
        for i in range(N_ESS_week):
            Constraints.append(soc_week[i, T_week] == soc_week[i, 0])
        ...
        C_week = (Op[0] * Cgrid_week + Op[1] * pun_abn_week + Op[3] * (cp.sum(cp.sum(Pbd_week)) + cp.sum(cp.sum(Pbc_week))) + Op[
            2] * cp.sum(cp.sum(Pgas_week)) + Op[5] * C_adj + 0 * flu)
        ...
        if result is None or result == float('inf') or result == float('-inf'):
            print(f"Week Optimization failed for day {m}")
            continue
        ...
        Pabn_week_value = np.array([Pabn_week[k].value.item() for k in range(T_week)])
        ...
        soc_week_Tra[m] = soc_week_value[0, 23]
        ...
        for k in range(T):
            ...
            Constraints.append(load_adj[0, k] == Load[k] - P_all[k])
        ...
        if result is None or result == float('inf') or result == float('-inf'):
            print(f"Day Optimization failed for day {m}")
            continue
